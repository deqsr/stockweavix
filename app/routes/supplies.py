...
def create_supply_contract():
    if request.method == 'POST':
        ...
        initial_status_name = "Новий"  # Або ваш дефолтний статус
        ...
        new_contract = SupplyContract(
            ...
            ContractPrice=decimal.Decimal('0.00')
        )
        db.session.add(new_contract)
        try:
            db.session.commit()

            flash_message = f'Поставку ID {new_contract.SupplyID} успішно створено! Тепер можна додати товари.'
            flash(flash_message, 'success')

            return redirect(url_for('supplies_bp.edit_supply_contract', supply_id=new_contract.SupplyID))
